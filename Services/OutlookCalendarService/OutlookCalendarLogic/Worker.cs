...
namespace OutlookCalendarLogic {
  public class Worker {
	  ...
	  if ((sslPolicyErrors & SslPolicyErrors.RemoteCertificateChainErrors) != 0) {
		if (chain != null) {
		  foreach (X509ChainStatus status in chain.ChainStatus) {
			if ((certificate.Subject == certificate.Issuer) && (status.Status == X509ChainStatusFlags.UntrustedRoot)) { // Self-signed certificates with an untrusted root are valid. 
			  continue;
			} else {
	...
	private bool DatabaseIsBeingServiced(ref OpenHRSystem openHRSystem, SqlConnection conn) {
	  ...
	  try {
		if (GetSystemSetting("outlook service 2", "running", conn) == "1") {//Is the service running?
		  try {
		  ...
		  if ((runServer == openHRSystem.ServiceServer)) {
			...
			RaiseMessageEvent(new MessageEventDetails(
				$"Database ({openHRSystem}) already being serviced by server {runServer}.", EventLogEntryType.Warning, MessageEventDetails.MessageEventType.WindowsEventsLog));
	...
	private bool DatabaseVersionIsOK(ref OpenHRSystem openHRSystem, SqlConnection conn) {
	  ...
	  if ((openHRSystem.VersionOk && (!returnBool))) {
		RaiseMessageEvent(new MessageEventDetails(
			$"Database ({openHRSystem}) is incompatible with the OpenHR Outlook Calendar Service 2 ({_svcVersion}). Contact your system administrator.", EventLogEntryType.Warning, MessageEventDetails.MessageEventType.WindowsEventsLog));
