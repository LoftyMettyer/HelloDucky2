...
(function (window, $) {
   ...
   var version = "1.0.0",
    ...
    resetSession = function () { //Some variables used in this function are global and declared in Site.Master
      ...
      readCookie = function (cookieValue) {
         var defer = $.Deferred();

         $.get("ReadCookie?CookieName=" + cookieValue, function (data, status) {
            defer.resolve(data);
         })
         return defer.promise();
      },
      deleteCookie = function (cookieValue) {
         //$.get("DeleteCookie?CookieName=" + cookieValue);
         $.ajax({
            url: "DeleteCookie?CookieName=" + cookieValue,
            type: "DELETE"
         });
         return true;
      }
