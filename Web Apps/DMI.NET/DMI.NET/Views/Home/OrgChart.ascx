...
<script>
   ...
   $(document).ready(function () {
      ...
      if ('<%=Model.any()%>' == 'False') {
         ...
         $("#hiddenItems").find(":hidden").not("script").each(function () {
            ...
            nodeHTML += '<div class="jobTitle truncate employeeData" >' + employeeJobTitle + '</div>';
            nodeHTML += '<img style="width: 48px; height: 48px;" src="' + photoPath + '"/>';
            nodeHTML += '<p class="employeeData truncate">' + employeeForenames + ' ' + employeeSurname + '</p>';
            nodeHTML += '<ul id="' + employeeStaffNo + '">';
            nodeHTML += '</li>';
            $('#' + parentNode).append(nodeHTML);
         });

         //Set tooltip to all div which has class employeeData
   ...
   function printOrgChart(pfPreview) {
      ...
      if (($('.printSelect:checked:enabled').length === 0) && (!fPrintAll)) {
         ...
         newWin.document.write('.truncate {width: 94%; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; min-height: 20px !important; margin:3px 2px 0px 2px;}');
         newWin.document.write('.jOrgChart .node {height: auto;width: auto;min-width: 150px;max-width: 150px;overflow:hidden;}');
         newWin.document.write('</sty');
         newWin.document.write('le>');
