...
<%@ Language=VBScript %>
<!--#INCLUDE FILE="include/svrCleanup" -->
<%
	Response.Expires = -1
%>
<HTML>
...
<!--
	...
	/*if ((frmPopup.txtLoading.value == 1)	
			|| (frmPopup.txtEventColumnsLoaded.value == 0)
			|| (frmPopup.txtHaveSetLookupValues.value == 0)
			|| ((frmEvent.LookupType.value == 1) 
					&& (frmPopup.txtLookupColumnsLoaded.value == 0)))
		{
...
<script LANGUAGE=JavaScript id=scptGeneralFunctions>
	...
	function populateEventColumns()
	{
		...
		var frmGetDataForm = window.parent.frames("calendardataframe").document.forms("frmGetCalendarData");
		var frmDef=document.parentWindow.parent.window.dialogArguments.parent.frames("workframe").document.forms("frmDefinition");	
	...
	function loadAvailableEventColumns()
	{
		...
		frmRefresh = document.parentWindow.parent.window.dialogArguments.parent.frames("pollframe").document.forms("frmHit");	
		...
		with (frmPopup)
		{
			...
			var frmUtilDefForm = window.parent.frames("calendardataframe").document.forms("frmCalendarData");
				...
				for (i=0; i<dataCollection.length; i++)  
				{
					...
					if (sControlName.substr(0, 10) == "txtRepCol_") 
					{
						...
						if (iTableIDControlValue == frmPopup.cboEventTable.options[frmPopup.cboEventTable.selectedIndex].value)
						{
							...
							{
								var oOption = document.createElement("OPTION");
								cboStartDate.options.add(oOption);
		...
		{
			ASRIntranetFunctions.MessageBox("The selected event table has no date columns. Please select an event table that contains date columns.",48,"Calendar Reports");
	...
	function populateEventTableCombo()
	{
		...
		var frmRefresh = document.parentWindow.parent.window.dialogArguments.parent.frames("pollframe").document.forms("frmHit");	
	...
	function validateEventInfo()
	{
		...
		if (sEventName == '' || sEventName.length < 1)
		{
			...
			ASRIntranetFunctions.MessageBox(sMessage,48,"Calendar Reports");
	...
	{	
		...
		var frmDef = document.parentWindow.parent.window.dialogArguments.parent.frames("workframe").document.forms("frmDefinition");
		var frmUse = document.parentWindow.parent.window.dialogArguments.parent.frames("workframe").document.forms("frmUseful");
	...
	function removeEventTable(piChildTableID)
	{
		...
		var frmUseful = document.parentWindow.parent.window.dialogArguments.parent.frames("workframe").document.forms("frmUseful");
		var frmDefinition = document.parentWindow.parent.window.dialogArguments.parent.frames("workframe").document.forms("frmDefinition");
		var frmOriginalDefinition = document.parentWindow.parent.window.dialogArguments.parent.frames("workframe").document.forms("frmOriginalDefinition");
		...
		if (frmUseful.txtLoading.value == 'N') 
		{
				...
				(frmUseful.txtSelectedColumnsLoaded.value == 0))) 
			{
					...
					//if (frmDefinition.ssOleDBGridChildren.Rows > 0)
				{
					...
					{
						iAnswer = ASRIntranetFunctions.MessageBox("One or more columns from the child table have been included in the report definition. Changing the child table will remove these columns from the report definition. Do you wish to continue ?",36,"Calendar Reports");
...
<BODY id=bdyMain name=bdyMain <%=session("BodyColour")%> leftmargin=20 topmargin=20 bottommargin=20 rightmargin=5>
...
%>									
	...
	<tr>
		<td valign=top width=50%>
			<table class="outline" cellspacing="0" cellpadding="4" width=100% height=100%>
				<tr height=5> 
					<td height=5 colspan=5 align=left valign=top>
						Event Start : <BR>
						<TABLE class="invisible" cellspacing="0" cellpadding="0" width=100%>
							<tr height=5>
								<td width=5></td>
	...
	<tr>
		<td valign=top width=50% rowspan=2>
			<table class="outline" cellspacing="0" cellpadding="4" width=100% height=100%>
				<tr height=10> 
					<td height=10 colspan=5 rowspan=2 align=left valign=top>
						Event End : <BR>
						<TABLE class="invisible" cellspacing="0" cellpadding="0" width=100%>
							<tr height=5>
								<td width=5></td>
							...
							<tr height=5>
								...
								<td width=5></td>
								<td>
									<select disabled="disabled" id=cboDuration name=cboDuration class="" style="WIDTH: 100%" class="combo combodisabled"
									    onchange="eventChanged();">
									</select>	
								</td>
								<td width=5></td>
