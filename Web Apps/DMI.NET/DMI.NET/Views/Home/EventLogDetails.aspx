...
<!--
    function eventlogdetails_window_onload() {
        ...
        {
            var frmOpenerRefresh = window.dialogArguments.parent.frames("workframe").document.forms("frmRefresh");
            var frmMainLog = window.dialogArguments.parent.frames("workframe").document.forms("frmLog");
		

            ASRIntranetFunctions.MessageBox("This record no longer exists in the event log.", 48, "Event Log");
        ...
        {
            var frmOpenerDetails = window.dialogArguments.parent.frames("workframe").document.forms("frmDetails");
	...
	with rsAllBatchJobs
		if not (.EOF and .BOF) then
			...
			do until .EOF
                ...
                Response.Write("<INPUT type=hidden Name=txtEventID_" & .Fields("ID") & " ID=txtEventID_" & .Fields("ID") & " VALUE=""" & Replace(.Fields("ID").Value, """", "&quot;") & """>" & vbCrLf)
                ...
                Response.Write("<INPUT type=hidden Name=txtEventName_" & .Fields("ID") & " ID=txtEventName_" & .Fields("ID") & " VALUE=""" & sValue & """>" & vbCrLf)
                Response.Write("<INPUT type=hidden Name=txtEventMode_" & .Fields("ID") & " ID=txtEventMode_" & .Fields("ID") & " VALUE=""" & Replace(.Fields("Mode").Value, """", "&quot;") & """>" & vbCrLf)
				
                Response.Write("<INPUT type=hidden Name=txtEventStartTime_" & .Fields("ID") & " ID=txtEventStartTime_" & .Fields("ID") & " VALUE=""" & convertSQLDateToLocale(.Fields("DateTime").Value) & " " & convertSQLDateToTime(.Fields("DateTime").Value) & """>" & vbCrLf)
				
                If IsDBNull(.Fields("EndTime").Value) Then
                    Response.Write("<INPUT type=hidden Name=txtEventEndTime_" & .Fields("ID") & " ID=txtEventEndTime_" & .Fields("ID") & " VALUE=""" & vbNullString & """>" & vbCrLf)
                Else
                    Response.Write("<INPUT type=hidden Name=txtEventEndTime_" & .Fields("ID") & " ID=txtEventEndTime_" & .Fields("ID") & " VALUE=""" & ConvertSqlDateToLocale(.Fields("EndTime").Value) & " " & ConvertSqlDateToTime(.Fields("EndTime").Value) & """>" & vbCrLf)
                End If
				
                Response.Write("<INPUT type=hidden Name=txtEventDuration_" & .Fields("ID") & " ID=txtEventDuration_" & .Fields("ID") & " VALUE=""" & objUtilities.FormatEventDuration(.Fields("Duration").Value) & """>" & vbCrLf)

                Response.Write("<INPUT type=hidden Name=txtEventType_" & .Fields("ID") & " ID=txtEventType_" & .Fields("ID") & " VALUE=""" & Replace(.Fields("Type").Value, """", "&quot;") & """>" & vbCrLf)
                Response.Write("<INPUT type=hidden Name=txtEventStatus_" & .Fields("ID") & " ID=txtEventStatus_" & .Fields("ID") & " VALUE=""" & Replace(.Fields("Status").Value, """", "&quot;") & """>" & vbCrLf)
                Response.Write("<INPUT type=hidden Name=txtEventUser_" & .Fields("ID") & " ID=txtEventUser_" & .Fields("ID") & " VALUE=""" & Replace(.Fields("Username").Value, """", "&quot;") & """>" & vbCrLf)
				
                Response.Write("<INPUT type=hidden Name=txtEventSuccessCount_" & .Fields("ID") & " ID=txtEventSuccessCount_" & .Fields("ID") & " VALUE=""" & Replace(.Fields("SuccessCount").Value, """", "&quot;") & """>" & vbCrLf)
                Response.Write("<INPUT type=hidden Name=txtEventFailCount_" & .Fields("ID") & " ID=txtEventFailCount_" & .Fields("ID") & " VALUE=""" & Replace(.Fields("FailCount").Value, """", "&quot;") & """>" & vbCrLf)
				
                Response.Write("<INPUT type=hidden Name=txtEventBatchRunID_" & .Fields("ID") & " ID=txtEventBatchRunID_" & .Fields("ID") & " VALUE=""" & Replace(.Fields("BatchRunID").Value, """", "&quot;") & """>" & vbCrLf)
                Response.Write("<INPUT type=hidden Name=txtEventBatchName_" & .Fields("ID") & " ID=txtEventBatchName_" & .Fields("ID") & " VALUE=""" & Replace(.Fields("BatchName").Value, """", "&quot;") & """>" & vbCrLf)
                Response.Write("<INPUT type=hidden Name=txtEventBatchJobID_" & .Fields("ID") & " ID=txtEventBatchJobID_" & .Fields("ID") & " VALUE=""" & Replace(.Fields("BatchJobID").Value, """", "&quot;") & """>" & vbCrLf)
				
                If Session("eventBatch") = True Then
                    If Session("eventID") = .Fields("ID") Then
                        Session("cboString") = Session("cboString") & "<OPTION SELECTED NAME='" & .Fields("Name") & "' VALUE='" & .Fields("ID") & "'>" & .Fields("Type").Value & " - " & .Fields("Name") & vbCrLf
                    Else
                        Session("cboString") = Session("cboString") & "<OPTION NAME='" & .Fields("Name") & "' VALUE='" & .Fields("ID") & "'>" & .Fields("Type").Value & " - " & .Fields("Name") & vbCrLf
                    End If
                End If
...
<%
	...
	if not (rsEventDetails.BOF and rsEventDetails.EOF) then
		do while not rsEventDetails.EOF
			...
			sValue = rsEventDetails.Fields("Notes")		'original value
			sValue = Replace(sValue, """", "&quot;")	'escape quotes
			
            Response.Write("<INPUT type=hidden Name=txtEventNotes_" & rsEventDetails.Fields("EventLogID") & "_" & iDetailCount & " ID=txtEventNotes_" & rsEventDetails.Fields("EventLogID") & "_" & iDetailCount & " VALUE=""" & sValue & """>" & vbCrLf)
...
<FORM id=frmEmail name=frmEmail method=post style="visibility:hidden;display:none" action="emailSelection.asp">
    ...
    <script type="text/javascript">
        ...
        window.onload = eventlogdetails_window_onload;
    </script>
