...
PRINT 'Step - Menu & Category enhancements'
	...
	EXECUTE sp_executesql N'CREATE PROCEDURE dbo.[spssys_getobjectcategories](@utilityType as integer, @UtilityID as integer)
		...
		END'
	GRANT EXECUTE ON dbo.[spssys_getobjectcategories] TO [ASRSysGroup];

	IF EXISTS (SELECT *	FROM dbo.sysobjects	WHERE id = object_id(N'[dbo].[spssys_saveobjectcategories]') AND xtype = 'P')
		DROP PROCEDURE [dbo].[spssys_saveobjectcategories];
	EXECUTE sp_executesql N'CREATE PROCEDURE dbo.[spssys_saveobjectcategories](@utilityType as integer, @UtilityID as integer, @CategoryID integer)
		AS
		BEGIN
