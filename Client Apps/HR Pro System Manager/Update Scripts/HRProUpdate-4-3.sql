...
PRINT 'Step - Populate code generation tables'
	...
	EXEC sp_executesql N'INSERT [dbo].[tbstat_componentcode] ([objectid], [code], [datatype], [name], [aftercode], [isoperator], [operatortype], [id], [casecount]) VALUES (N''a9997816-add0-467f-999d-79ef30c2b713'', N''(CASE WHEN ISNULL(DATALENGTH({0}),0) = 0 THEN 1 ELSE 0 END)'', 3, N''Is Field Empty'', NULL, 0, 0, 16, 1)';
	EXEC sp_executesql N'INSERT [dbo].[tbstat_componentcode] ([objectid], [code], [datatype], [name], [aftercode], [isoperator], [operatortype], [id], [casecount]) VALUES (N''8caf9f74-dee4-4618-8d59-e292847f202a'', N''(CASE WHEN ISNULL(DATALENGTH({0}),0) > 0 THEN 1 ELSE 0 END)'', 3, N''Is Field Populated'', NULL, 0, 0, 61, 1)';
