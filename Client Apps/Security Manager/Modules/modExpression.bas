...
Public Enum ExpressionTypes
  ...
  giEXPR_MATCHWHEREEXPRESSION = 17  'Data Manager Only
  giEXPR_RECORDINDEPENDANTCALC = 18 'Data Manager Only
...
Public Const giPRINT_XCOLUMN = 3000         '2600 = 4 columns
...
Private mbIsConnected As Boolean                      ' Is a connection to the database established
'Public gADOExpressionConnection As ADODB.Connection   ' Connection to the datbase
...
'  Set rsExpr = rdoCon.OpenResultset(sSQL)
...
Public Function GetExprField(lngExprID As Long, sField As String) As Variant
  ...
  rsExpr.Open sSQL, gADOCon, adOpenKeyset, adLockReadOnly
...
ErrorTrap:
  MsgBox "Error retrieving field value from database.", vbOKOnly + vbCritical, App.Title
  Resume TidyUpAndExit
...
ErrorTrap:
  MsgBox "Error setting Expression Access.", vbOKOnly + vbCritical, App.Title
  Resume TidyUpAndExit
...
ErrorTrap:
  ...
  MsgBox "Error checking owner of expression.", vbOKOnly + vbExclamation, App.Title
  Resume TidyUpAndExit
...
ErrorTrap:
  isOwnerOfComp = False
  MsgBox "Error checking owner of component.", vbOKOnly + vbExclamation, App.Title
  Resume TidyUpAndExit
...
ErrorTrap:
  isCompHidden = True
  MsgBox "Error checking access of component.", vbOKOnly + vbExclamation, App.Title
  Resume TidyUpAndExit
...
ErrorTrap:
  isCompDeleted = True
  MsgBox "Error checking if component has been deleted.", vbOKOnly + vbExclamation, App.Title
  Resume TidyUpAndExit
...
'********************************************************************************
  ...
  If bIsDeleted Then
    ...
    If bShowMessages Then
      MsgBox "The selected component has been made deleted by another user. " _
  ...
  If bIsHidden Then
    ...
    If bIsExprOwner Then
      ...
      If bIsCompOwner Then
        ...
        If objComp.ParentExpression.Access <> ACCESS_HIDDEN Then
          ...
          If bShowMessages Then
            MsgBox "The selected component is hidden, " & _
      ...
      Else
        ...
        If bShowMessages Then
          MsgBox "The selected component is owned by another user and " & vbCrLf & _
    ...
    Else
      ...
      If bShowMessages Then
        MsgBox "The selected component is hidden and cannot be added to " & _
...
ErrorTrap:
  ValidComponent = -1
  MsgBox "Error checking validity of component.", vbOKOnly + vbExclamation, App.Title
  Resume TidyUpAndExit
...
'********************************************************************************
  ...
  If bShowMessages Then
    ...
    Case 1:
      sMessage = "The selected " & ExpressionTypeName(objExpr.ExpressionType) & " contains hidden components. " & vbCrLf & _
                  "The " & ExpressionTypeName(objExpr.ExpressionType) & " will now be made hidden."
    Case 2:
      sMessage = "The selected " & ExpressionTypeName(objExpr.ExpressionType) & " contains hidden components and is " & _
                  "owned by '" & objExpr.Owner & "'. " & vbCrLf & _
                  "The " & ExpressionTypeName(objExpr.ExpressionType) & " will now be made hidden."
    Case 3:
      sMessage = "The selected " & ExpressionTypeName(objExpr.ExpressionType) & " contains deleted components. " & vbCrLf & _
                  "Please remove the '<unknown>' components from the " & ExpressionTypeName(objExpr.ExpressionType) & "."
    Case 4:
      sMessage = "The selected " & ExpressionTypeName(objExpr.ExpressionType) & " contains hidden components which " & _
                  "are owned by another user. " & vbCrLf & "These components will " & _
                  "now be removed from the " & ExpressionTypeName(objExpr.ExpressionType) & "."
    End Select
...
ErrorTrap:
  ValidateExpr = -1
  MsgBox "Error checking validity of expression.", vbOKOnly + vbExclamation, App.Title
  Resume TidyUpAndExit
...
'********************************************************************************
  ...
  For Each objComp In objExpr.Components
    With objComp
      If isCompHidden(objComp) And (Not isOwnerOfComp(objComp)) Then
...
ErrorTrap:
  ...
  MsgBox "Error removing hidden components.", vbOKOnly + vbExclamation, App.Title
  Resume TidyUpAndExit
...
'Public Function OpenRecordset(sSQL As String, Optional CursorType As ADODB.CursorTypeEnum, _
'  Optional LockType As ADODB.LockTypeEnum) As ADODB.Recordset
'
...
'  If IsMissing(CursorType) Then CursorType = adOpenDynamic
'  If IsMissing(LockType) Then LockType = adLockReadOnly
...
''''    sConnectString = "Driver={SQL Server};Server=" & gsSQLServerName & ";UID=" & gsUserName & ";PWD=" & gsPassword & ";Database=" & Database.DatabaseName & ";Integrated Security=SSPI;"
''''  Else
''''    sConnectString = "Driver={SQL Server};Server=" & gsSQLServerName & ";UID=" & gsUserName & ";PWD=" & gsPassword & ";Database=" & Database.DatabaseName
''''  End If
''''
''''  Set gADOExpressionConnection = New ADODB.Connection
''''  With gADOExpressionConnection
...
'''' Can't call it CloseConnection as it will cause a conflict with audit stuff
'''Public Sub ConnectionClose()
'''
'''  If Not gADOExpressionConnection Is Nothing Then
'''    gADOExpressionConnection.Close
'''    gADOExpressionConnection.Cancel
'''    Set gADOExpressionConnection = Nothing
'''  End If
'''
'''  mbIsConnected = False
'''
...
Public Function GetColumnName(plngColumnID As Long) As String
    ...
    GetColumnName = rsLookup.Fields(0).Value
    Set rsLookup = Nothing

End Function

Public Function GetTableName(plngTableID As Long) As String
    ...
    rsLookup.Open sSQL, gADOCon, adOpenForwardOnly, adLockReadOnly
    GetTableName = rsLookup.Fields(0).Value
    Set rsLookup = Nothing
