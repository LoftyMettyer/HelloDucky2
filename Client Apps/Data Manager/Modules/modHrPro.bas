...
Public Function CheckLinkedServerIsValid(Server As String, Catalog As String, Table As String) As Boolean
  ...
  bOK = True
  sSQL = "SELECT * FROM sys.sysservers WHERE srvname = '" & Server & "'"
  
  Set rsTemp = datGeneral.GetRecords(sSQL)
  bOK = Not (rsTemp.EOF And rsTemp.BOF)
  
  If bOK Then
    sSQL = "SELECT * FROM [" & Server & "].master.sys.databases WHERE name = '" & Catalog & "'"
    Set rsTemp = datGeneral.GetRecords(sSQL)
    bOK = Not (rsTemp.EOF And rsTemp.BOF)
  End If
  
  If bOK Then
    sSQL = "SELECT * FROM [" & Server & "].[" & Catalog & "].sys.tables WHERE name = '" & Table & "'"
    Set rsTemp = datGeneral.GetRecords(sSQL)
    bOK = Not (rsTemp.EOF And rsTemp.BOF)
  End If
  
  CheckLinkedServerIsValid = bOK
  rsTemp.Close
  Set rsTemp = Nothing
