...
Private Function PasswordChange() As Boolean
  ...
  'JPD 20060130 Fault 10745
  ' Check if any recedit/find windows open
  If fOK Then
    fRecFormOpen = False
  ...
  Else
    strOldPassword = IIf(Len(txtOldPW.Text) = 0, "''", "'" & Replace(txtOldPW.Text, "'", "''") & "'")
    strNewPassword = IIf(Len(txtNewPW.Text) = 0, "''", "'" & Replace(txtNewPW.Text, "'", "''") & "'")
  End If
  ...
  If gADOCon.Errors.Count > 0 Then
    For iCount = 0 To gADOCon.Errors.Count - 1
      ...
      Select Case gADOCon.Errors(iCount).NativeError
        ...
        Case 18463, 15115
          mstrErrorMessage = "The password does not meet policy requirements because it has been used in the recent past."
        
        Case 18464, 15116
          mstrErrorMessage = "The password does not meet policy requirements because it is too short."
        
        Case 18465, 15117
          mstrErrorMessage = "The password does not meet policy requirements because it is too long."
        
        Case 18466, 15118
          mstrErrorMessage = "The password does not meet policy requirements because it is not complex enough."
        
        Case 18467, 15119
          mstrErrorMessage = "The password does not meet the requirements of the password filter DLL."
...
Private Sub Relogin()
  ...
  gsConnectionString = "Driver=SQL Server;" & _
  ...
  With gADOCon
    .ConnectionString = gsConnectionString
    .Provider = "SQLOLEDB"
    .CommandTimeout = 0
    .ConnectionTimeout = 0
