...
INSERT ASRSysTableTriggers (TriggerID, TableID, Name, CodePosition, IsSystem, Content) VALUES (1, 251, 'Update Absence Breakdown', 0, 1, '	DELETE [dbo].[tbuser_Absence_Breakdown] WHERE [id_251] IN (SELECT DISTINCT [id] FROM deleted);
	...
	INSERT Absence_Breakdown([source], ID_251, Post_ID, [Type], Payroll_Type_Code, Reason, Payroll_Reason_Code, Absence_In, Duration, Absence_Date, [Session]
		...
		SELECT ''post'', i.ID, ap.Post_ID, i.Absence_Type, ISNULL(at.Payroll_Code, ''''), i.Reason, ISNULL(ar.Payroll_Code, ''''), wp.Absence_In
...
INSERT ASRSysTableTriggers (TriggerID, TableID, Name, CodePosition, IsSystem, Content) VALUES (2, 250, 'Update Absence Breakdown', 0, 1, '    DELETE [dbo].[tbuser_Absence_Breakdown] WHERE [id_250] IN (SELECT DISTINCT [id] FROM deleted);
	...
	INSERT Absence_Breakdown([source], ID_250, Post_ID, [Type], Payroll_Type_Code, Reason, Payroll_Reason_Code, Absence_In, Duration, Absence_Date, [Session]
		, Day_Pattern_AM, Day_Pattern_PM, Hour_Pattern_AM, Hour_Pattern_PM, Staff_Number, Payroll_Company_Code)	
		SELECT ''pers'', i.ID, ap.Post_ID, i.Absence_Type, ISNULL(at.Payroll_Code, ''''), i.Reason, ISNULL(ar.Payroll_Code, ''''), wp.Absence_In
			, dbo.udfsysDurationFromPattern(wp.Absence_In, dr.IndividualDate, dr.SessionType, wp.Sunday_Hours_AM, wp.Monday_Hours_AM, wp.Tuesday_Hours_AM
...
INSERT ASRSysTableTriggers (TriggerID, TableID, Name, CodePosition, IsSystem, Content) VALUES (3, 252, 'Absence Rollup', 0, 1, '	DECLARE @thisAbsenceID	integer,
			@startDate	datetime,
			@endDate	datetime;
