...
GO
  ...
  INSERT ASRSysTableTriggers (TriggerID, TableID, Name, CodePosition, IsSystem, Content) VALUES (10, 242, 'Maintain Allowance on Personnel Records', 1, 1, '    MERGE INTO [dbo].[tbuser_Allowances] AS Target
		USING (SELECT a.[ID_1], i.[Type], i.[Effective_Date], i.[End_Date], i.[Frequency], i.[Amount], i.[Currency], a.[Post_ID], i.[ApptAllID]
			FROM [dbo].[tbuser_Appointment_Allowances] aa
			INNER JOIN inserted i ON i.[ID] = aa.[ID]
			INNER JOIN [dbo].[Appointments] a ON a.[ID] = i.[ID_3]
			WHERE i.[ApptAllID] > 0)
		AS Source ([ID_1], [Type], [Effective_Date], [End_Date], [Frequency], [Amount], [Currency], [Post_ID], [ApptAllID])
 ...
 INSERT ASRSysTableTriggers (TriggerID, TableID, Name, CodePosition, IsSystem, Content) VALUES (11, 243, 'Maintain Benefits on Personnel Records', 1, 1, '    MERGE INTO [dbo].[tbuser_Benefits] AS Target
		USING (SELECT a.[ID_1], i.[Type], i.[Effective_Date], i.[End_Date], i.[Frequency], i.[Cost], i.[Currency], i.[Provider], i.[Level_of_Cover], i.[Reference_Number]
			, i.[Percentage_to_Employer], i.[Annual_Cost_to_Employer], a.[Post_ID], i.[ApptBenID]
		...
		AS Source ([ID_1], [Type], [Effective_Date], [End_Date], [Frequency], [Cost], [Currency], [Provider], [Level_of_Cover], [Reference_Number]
			, [Percentage_to_Employer], [Annual_Cost_to_Employer], [Post_ID], [ApptBenID])
	...
	WHEN MATCHED THEN
		UPDATE SET [Type] = Source.[Type], [Effective_Date] = Source.[Effective_Date], [End_Date] = Source.[End_Date], [Frequency] = Source.[Frequency], [Cost] = Source.[Cost], [Currency] = Source.[Currency]
			...
			, [Annual_Cost_to_Employer] = Source.[Annual_Cost_to_Employer]
	WHEN NOT MATCHED BY TARGET THEN
 ...
 INSERT ASRSysTableTriggers (TriggerID, TableID, Name, CodePosition, IsSystem, Content) VALUES (12, 244, 'Maintain Deductions on Personnel Records', 1, 1, '    MERGE INTO [dbo].[tbuser_Deductions] AS Target
		USING (SELECT a.[ID_1], i.[Type], i.[Effective_Date], i.[End_Date], i.[Frequency], i.[Amount], i.[Currency], i.[Reference], a.[Post_ID], i.[ApptDedID]
			FROM [dbo].[tbuser_Appointment_Deductions] ad
			INNER JOIN inserted i ON i.[ID] = ad.[ID]
			INNER JOIN [dbo].[Appointments] a ON a.[ID] = i.[ID_3]
			WHERE i.[ApptDedID] > 0)
		AS Source ([ID_1], [Type], [Effective_Date], [End_Date], [Frequency], [Amount], [Currency], [Reference], [Post_ID], [ApptDedID])
