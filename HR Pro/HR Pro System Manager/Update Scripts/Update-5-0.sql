...
PRINT 'Step 1 - System procedures'
	...
	EXECUTE sp_executeSQL N'CREATE PROCEDURE dbo.spstat_scriptnewcolumn (@columnid integer OUTPUT, @tableid integer, @columnname varchar(255)
		, @datatype integer, @description varchar(255), @size integer, @decimals integer, @islocked bit)
	AS
	BEGIN
		...
		INSERT dbo.[tbsys_scriptedobjects] ([guid], [objecttype], [targetid], [ownerid], [effectivedate], [revision], [locked], [lastupdated])
			SELECT NEWID(), 2, @columnid, ''AAAAAAAA-BBBB-CCCC-DDDD-EEEEEEEEEEEE'', ''01/01/1900'',1,@islocked, GETDATE()
...
PRINT 'Step 12 - Auto Configuration'
	...
	BEGIN
		...
		SET @columnid = 0;
		EXECUTE dbo.spstat_scriptnewcolumn @columnid OUTPUT, @perstableid, 'Mobile_Workflow_Activated', -7, 'Checkbox if the workflow mobile is activated for this user', 1, 0, 1;
		...
		EXECUTE dbo.spstat_setdefaultmodulesetting 'MODULE_MOBILE', 'Param_UniqueEmailColumn', @columnid, 'PType_ColumnID';

		SELECT @columnid = ISNULL([parametervalue],0) FROM dbo.[ASRSysModuleSetup] WHERE ModuleKey = 'MODULE_PERSONNEL' AND ParameterKey = 'Param_FieldsLoginName' AND [ParameterType] = 'PType_ColumnID';
		EXECUTE dbo.spstat_setdefaultmodulesetting 'MODULE_MOBILE', 'Param_FieldsLoginName', @columnid, 'PType_ColumnID';
