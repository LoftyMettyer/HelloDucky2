...
--Create New SSI Dashboard Stored Procedures
	...
	IF EXISTS (SELECT *
		FROM dbo.sysobjects
		WHERE id = object_id(N'[dbo].[spASRIntGetChartData]')
			AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
		DROP PROCEDURE [dbo].[spASRIntGetChartData];

	SET @sSPCode = 'CREATE PROCEDURE [dbo].[spASRIntGetChartData]
		AS
		BEGIN
			DECLARE @iDummy integer;
		END';
	...
	SET @sSPCode = 'ALTER PROCEDURE [dbo].[spASRIntGetChartData] (
