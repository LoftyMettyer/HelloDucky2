...
Private Function AccessState(lngExprID As Long) As String
 ...
 ' Returns access code for the current expression.
  ...
  Set rsTemp = datData.OpenRecordset(strSQL, adOpenStatic, adLockReadOnly)
  ...
  If CheckForDeletedCalcs Then
    If CheckExpression(True) Then
        ...
        If mobjExpression.ExpressionID > 0 And LCase(mobjExpression.Owner) = LCase(gsUserName) And Me.optAccess(2).Value = True And mblnWasHidden = False Then
          If (mobjExpression.ExpressionType = giEXPR_RUNTIMECALCULATION) Or (mobjExpression.ExpressionType = giEXPR_RECORDINDEPENDANTCALC) Then      ' Its a Runtime Calc Expression
            If datGeneral.CheckCanMakeHidden("E", mobjExpression.ExpressionID, gsUserName, "Expression Validation") = False Then
              mobjExpression.mfDontUpdateTimeStamp = False
              Exit Sub
            End If
          Else                                          ' Its a Filter Expression
            If datGeneral.CheckCanMakeHidden("F", mobjExpression.ExpressionID, gsUserName, "Filter Validation") = False Then
              mobjExpression.mfDontUpdateTimeStamp = False
              Exit Sub
            End If
          End If
...
Private Function CheckExpression(pfFullValidation As Boolean) As Boolean
  ...
  If fValid And (mobjExpression.ExpressionID > 0) Then
    ...
    Else
      ...
      fReadOnly = (UCase(rsCheck!UserName) <> UCase(gsUserName) And _
        (rsCheck!Access <> ACCESS_READWRITE)) And _
  ...
  If fValid And _
    (mobjExpression.ExpressionType <> giEXPR_UTILRUNTIMEFILTER And _
    mobjExpression.ExpressionType <> giEXPR_MATCHJOINEXPRESSION And _
    mobjExpression.ExpressionType <> giEXPR_MATCHWHEREEXPRESSION And _
    mobjExpression.ExpressionType <> giEXPR_MATCHSCOREEXPRESSION) Then
...
Private Sub cmdTest_Click()
  ...
  If CheckExpression(False) = True Then
    ...
    'The reason for this is, if there are prompted values in the calc
    'then we want to view this and select an option so that the record
    'count we get is correct
    
    'fOK = mobjExpression.RuntimeFilterCode(strFilterCode, True, True)
    ...
    'If fOK Then
    If fOK And gcoTablePrivileges.Item(mobjExpression.BaseTableName).AllowSelect Then
  
      ' Create dynamic User defined functions
      UDFFunctions mvarUDFsRequired, True
  
      'strSQL = "SELECT COUNT(*) FROM " & _
               gcoTablePrivileges.Item(mobjExpression.BaseTableName).RealSource & _
               " WHERE ID IN (" & strFilterCode & ")"
      strSQL = "SELECT COUNT(ID) FROM " & _
               gcoTablePrivileges.Item(mobjExpression.BaseTableName).RealSource & _
               " WHERE ID IN (" & strFilterCode & ")"
...
'    End If
    ...
    If Not Me.Cancelled Then
      ...
      If mfModifiable And .ExpressionType = giEXPR_RUNTIMEFILTER Then
        mfModifiable = datGeneral.SystemPermission("FILTERS", "EDIT")
      End If
      ...
      If mfModifiable And .ExpressionType = giEXPR_RUNTIMECALCULATION Then
        mfModifiable = datGeneral.SystemPermission("CALCULATIONS", "EDIT")
      End If
...
Private Sub ConfigureScreen()
  ...
  Select Case Me.Caption
        Case "Filter Definition"
            Me.HelpContextID = 1109
        Case "Expression Definition"
            Me.HelpContextID = 1110
        Case "Calculation Definition"
            'Me.HelpContextID = 1111
            Me.HelpContextID = 1056
  End Select
  
  If Me.Caption = "Filter Definition" Then
  ...
  If Not mfModifiable Then
    ...
    txtDescription.ForeColor = vbGrayText
  End If

  ' JDM - 16/03/01 - Fault 1935 - Restore last view
  Select Case GetUserSetting("ExpressionBuilder", "ViewColours", EXPRESSIONBUILDER_COLOUROFF)
  ...
  Case giEXPR_UTILRUNTIMEFILTER, giEXPR_MATCHJOINEXPRESSION, giEXPR_MATCHWHEREEXPRESSION
  ...
  Select Case mobjExpression.ExpressionType
  Case giEXPR_UTILRUNTIMEFILTER, _
       giEXPR_MATCHJOINEXPRESSION, _
       giEXPR_MATCHWHEREEXPRESSION, _
       giEXPR_MATCHSCOREEXPRESSION
...
Private Sub sstrvComponents_ValidateSelection(SelectionType As SSActiveTreeView.Constants_ValidateSelection, StartNode As SSActiveTreeView.SSNode, EndNode As SSActiveTreeView.SSNode, Cancel As SSActiveTreeView.SSReturnBoolean)
  'JPD 20040913 Fault 9128
'  If (SelectionType = ssatValidateToggle) _
'    And (sstrvComponents.SelectedNodes.Count = 1) Then Cancel = True

  ' JDM - 21/07/05 - Fault 10182 - Couldn't multi-select with the ctrl key on nodes
...
Public Sub SetInitialExpandedNodes()
    ...
    Select Case GetUserSetting("ExpressionBuilder", "NodeSize", EXPRESSIONBUILDER_NODESMINIMIZE)
...
Private Sub Form_Resize()
  ...
  'NHRD27102004 Fault 8305 changed from height of 5820
'  If Me.ScaleHeight < 6300 Then
'    Me.Height = (Me.Height - Me.ScaleHeight) + 6300
'  End If
'
'  If Me.ScaleWidth < 7155 Then
'    Me.Width = (Me.Width - Me.ScaleWidth) + 7155
'  End If
