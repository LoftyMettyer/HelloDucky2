...
AS
	SELECT ID				AS [ID_Contract]
		, ID_1				AS [ID_Staff]
		, grade				AS [contractName]
		, department		AS [department]
		, location			AS [primarySite]
		, actual_hours		AS [contractedHoursPerWeek]
		, standard_hours	AS [maximumHoursPerWeek]
		, start_date		AS [effectiveFrom]
		, end_date			AS [effectiveTo]
		, [_deleted]		AS [isRecordInactive]
FROM dbo.tbuser_Salary;
GO
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			-- Force Contracts
			DECLARE temp cursor FOR
				SELECT TOP 1 ID FROM Salary
					WHERE ID_1 = @RecordID AND [start_date] <= DATEADD(dd, 0, DATEDIFF(dd, 0, GETDATE())) 
	...
	IF @TableID = 3
	BEGIN
		SELECT @parentID = ID_1 FROM dbo.Salary WHERE id = @RecordID;
		EXEC fusion.[pSendFusionMessageCheckContext] @MessageType='StaffChange', @LocalId=@parentID;
