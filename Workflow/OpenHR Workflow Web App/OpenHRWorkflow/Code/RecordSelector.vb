...
Public Class RecordSelector
	...
	Protected Overrides Sub Render(ByVal writer As HtmlTextWriter)
		...
		If HeaderRow IsNot Nothing Then
			...
			_visibleColumnCount = HeaderRow.Cells.Count

			customHeader.ApplyStyle(HeaderStyle)

			For iColCount = 0 To HeaderRow.Cells.Count - 1
				Dim sColumnCaption = UCase(customHeader.Cells(iColCount).Text)

				If _
				 (Not IsLookup And (sColumnCaption = "ID" Or (Left(sColumnCaption, 3) = "ID_" And Val(Mid(sColumnCaption, 4)) > 0))) Or (IsLookup And sColumnCaption.StartsWith("ASRSYS")) Then
					_visibleColumnCount = _visibleColumnCount - 1
				...
				Else
					...
					customHeader.Cells(iColCount).ID = ID.ToString & "header" & CStr(iColCount + 1)
					customHeader.Cells(iColCount).Style.Add("width", Unit.Pixel(DefaultColumnWidth).ToString)
					...
					With ctlTextPanel
						...
						If Rows.Count > 1 Then
							If IsLookup Then
								.Attributes("onclick") =
								 ("$get('txtActiveDDE').value='" & ID.Replace("Grid", "dde") &
								  "';try{setPostbackMode(3);}catch(e){};if(event.ctrlKey){__doPostBack('" & UniqueID & "','Sort$" &
								  customHeader.Cells(iColCount).Text & "+');}else{__doPostBack('" & UniqueID & "','Sort$") &
								 customHeader.Cells(iColCount).Text & "');}"
							Else
								.Attributes("onclick") =
								 ("try{setPostbackMode(3);}catch(e){};if(event.ctrlKey){__doPostBack('" & UniqueID & "','Sort$" &
								  customHeader.Cells(iColCount).Text & "+');}else{__doPostBack('" & UniqueID & "','Sort$") &
		...
		writer.Write("<div ID='" & ID.ToString.Replace("Grid", "") & "' " &
		 If(IsLookup, "style='", "style='position:absolute;") &
		 " width:" & CalculateWidth() & ";height:" & CalculateHeight() & ";" &
		 If(IsLookup, "", "top:" & Style.Item("TOP").ToString & ";") &
		 If(IsLookup, "", "left:" & Style.Item("LEFT").ToString & ";") &
		 sDivStyle &
		 ";overflow:hidden;border-color:black;border-style:solid;border-width:1px;background-color:" &
		 General.GetHtmlColour(
		Drawing.ColorTranslator.ToOle(Drawing.Color.FromArgb(customHeader.BackColor.A,
		...
		writer.Write("<table ID='" & ID.ToString.Replace("Grid", "") & "Header'" &
		 " style='position:absolute;top:0px;left:0px;width:" & CalculateGridWidth() & ";height:" &
		 CalculateHeaderHeight() &
		 ";table-layout:fixed;border:0'" &
		 " cellspacing='" & CellSpacing.ToString() & "'" &
		 If((HeaderRow IsNot Nothing And IsEmpty = False), " class='resizable'", "") &
		 ">")
		...
		writer.Write(
		 "<div id='" & ClientID.Replace("Grid", "") & "gridcontainer'  style='position:absolute;top:" &
		 CalculateHeaderHeight() &
		 ";bottom:" & CalculatePagerHeight() & ";left:0px;overflow-x:scroll;overflow-y:scroll;" &
		 "width:" & CalculateWidth() & ";" & "background-color:" &
		 General.GetHtmlColour(Drawing.ColorTranslator.ToOle(Drawing.Color.FromArgb(BackColor.A,
		  BackColor.R,
		  BackColor.G,
		  BackColor.B))) &
		...
		Attributes.CssStyle("WIDTH") = CalculateGridWidth()
		...
		Style.Add("border-top", "solid 1px black")
		Style.Add("border-bottom", "solid 1px black")
		AdjustWidthForScrollbar()
		' reduce width if vertical scrollbar

		MyBase.Render(writer)
		writer.Write("</div>")
		' Render the custom pager row now.
		If customPager IsNot Nothing Then
			writer.Write("<div  border='0' cellspacing='" & CellSpacing.ToString() & "' cellpadding='" &
			 CellPadding.ToString() & "' style='width:100%;position:absolute;right:0px;bottom:0px;height: " &
			 CalculatePagerHeight() & ";'>")

			customPager.ApplyStyle(PagerStyle)
	...
	Private Function CalculateWidth() As String
		...
		If Not IsLookup Then
			If Not Width.IsEmpty Then
				strWidth = String.Format("{0}{1}", Width.Value, (If((Width.Type = UnitType.Percentage), "%", "px")))
			End If
		Else
			Dim iGridWidth As Integer = _visibleColumnCount * (defaultColumnWidth + 2)
			' 2 = padding
			iGridWidth = CInt(If(iGridWidth < 0, 1, iGridWidth))
			iGridWidth = CInt(If(iGridWidth < Width.Value, Width.Value, iGridWidth))
			...
			If iGridWidth < 250 Then iGridWidth = 250
			' minimum width to ensure paging controls fit.

			strWidth = String.Format("{0}{1}", iGridWidth, (If((Width.Type = UnitType.Percentage), "%", "px")))
		End If
	...
	Private Function CalculateHeight() As String
		...
		Else
			...
			Dim iDropHeight As Integer =
			 (iRowHeight * CInt(If(Rows.Count > MaxDropDownRows, MaxDropDownRows, Rows.Count))) + 1

			iDropHeight += iRowHeight
			' add row for headers
			iDropHeight += 30
			' Pager height - now it's always displayed.
	...
	Private Function CalculateGridWidth() As String
		...
		If Not IsLookup Then
			If Not Width.IsEmpty Then
				strWidth = String.Format("{0}{1}", Width.Value - iScrollBarWidth,
				 (If((Width.Type = UnitType.Percentage), "%", "px")))
			End If
		Else
			Dim iGridWidth As Integer = _visibleColumnCount * (defaultColumnWidth + 2)
			' 2 = padding
			...
			strWidth = String.Format("{0}{1}", iGridWidth - iScrollBarWidth,
			 (If((Width.Type = UnitType.Percentage), "%", "px")))
		End If
	...
	Private Function CalculateHeaderHeight() As String
		...
		Dim strHeaderHeight As String
		Dim iHeaderHeight As Integer
		Dim iGridHeight As Integer = ControlHeight

		If NullSafeBoolean(ColumnHeaders) And (NullSafeInteger(HeadLines) > 0) Then
			Dim iGridTopPadding As Integer = CInt(NullSafeSingle(HeadFontSize) / 8)

			iHeaderHeight = CInt(((NullSafeSingle(HeadFontSize) + iGridTopPadding) * NullSafeInteger(HeadLines) * 2) _
			 - 2 _
			 - (NullSafeSingle(HeadFontSize) * (NullSafeInteger(HeadLines) + 1) * (iGridTopPadding - 1) / 4))
	...
	Private Function CalculatePagerHeight() As String
		...
		If (Width.Value < 175 And PageCount < 1) Or IsEmpty Then
			Return "0px"
		Else
			Return "24px"
		End If
	...
	Private Sub RecordSelector_DataBound(ByVal sender As Object, ByVal e As EventArgs) Handles Me.DataBound
		Height = CInt(RowStyle.Height.Value) * Rows.Count
	End Sub
	...
	Private Function ColumnSortDirectionCode(ByVal sColName As String) As Integer
		...
		Dim aColList() As String = Split(SortBy, ",")
		...
		If e.Row.RowType = DataControlRowType.Header Then
			...
			For Each tcTableCell In e.Row.Cells
				If AllowSorting Then
					...
					If HeaderStyle.Height.Value < 21 Then HeaderStyle.Height = Unit.Pixel(NullSafeInteger(HeadFontSize) * 2)
					tcTableCell.ApplyStyle(HeaderStyle)
				Else
					tcTableCell.Text &= ""
					If HeaderStyle.Height.Value < 21 Then HeaderStyle.Height = Unit.Pixel(NullSafeInteger(HeadFontSize) * 2)
					tcTableCell.ApplyStyle(HeaderStyle)
	...
	Private Sub RecordSelector_RowDataBound(ByVal sender As Object, ByVal e As GridViewRowEventArgs) Handles Me.RowDataBound
		...
		Try
			If e.Row.RowType = DataControlRowType.DataRow Then
				...
				For iColCount As Integer = 0 To e.Row.Cells.Count - 1
					...
					Dim sColumnCaption As String = UCase(HeaderRow.Cells(iColCount).Text)
					...
					If HeaderRow.Cells(iColCount).Text <> vbNullString Then

						curSelDataType = DataBinder.Eval(e.Row.DataItem, HeaderRow.Cells(iColCount).Text).GetType.ToString.ToUpper

						If curSelDataType.Contains("INT") OrElse curSelDataType.Contains("DECIMAL") OrElse curSelDataType.Contains("SINGLE") OrElse
						 curSelDataType.Contains("DOUBLE") Then curSelDataType = "Integer"
			...
			ElseIf e.Row.RowType = DataControlRowType.Header Then
				...
				For iColCount As Integer = 0 To e.Row.Cells.Count - 1
					...
					If sColumnCaption.ToUpper = "ASRSYSLOOKUPFILTERVALUE" Then
						Attributes.Remove("LookupFilterColumn")
						Attributes.Add("LookupFilterColumn", iColCount.ToString)
					End If
				Next
	...
	Public Property SelectedID(ByVal gv As GridView) As Integer
		Get
			If gv.Attributes("SelectedID") Is Nothing Then
				Return -1
			Else
				Return CInt(gv.Attributes("SelectedID"))
			End If
		End Get

		Set(ByVal value As Integer)
			gv.Attributes("SelectedID") = CStr(value)
		End Set
	End Property

	Private Sub RecordSelector_SelectedIndexChanging(ByVal sender As Object, ByVal e As GridViewSelectEventArgs) Handles Me.SelectedIndexChanging

		' Get the current page and row index numbers
		Dim iNewSelectedIndex As Integer = (PageIndex * PageSize) + e.NewSelectedIndex
	...
	Private Sub RecordSelector_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles Me.SelectedIndexChanged

		' Set the dropdown value to the selected item
		If IsLookup Then

			Dim ddl = DirectCast(Parent.FindControl(ID.Replace("Grid", "TextBox")), TextBox)

			ddl.Text = HttpUtility.HtmlDecode(Rows(SelectedIndex).Cells(CInt(ddl.Attributes("LookupColumnIndex"))).Text).ToString().Replace(Chr(160), Chr(32))
		End If
	End Sub
	...
	Private Sub RecordSelector_PageIndexChanging(ByVal sender As Object, ByVal e As GridViewPageEventArgs) Handles Me.PageIndexChanging
		...
		Dim dataTable = TryCast(HttpContext.Current.Session(ID.Replace("Grid", "DATA")), DataTable)
	...
	Private Sub RecordSelector_PageIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles Me.PageIndexChanged
		...
		If IsLookup Then
			For itemIndex As Integer = 0 To Rows.Count - 1
				iCurrentIndex = (PageIndex * PageSize) + itemIndex
				If SelectedID(Me) = iCurrentIndex Then
					SelectedIndex = itemIndex
					DataBind()
					' Need to bind (again) to reset selection.
					Exit For
				End If
			Next
		Else
		...
		If dataTable IsNot Nothing Then
			...
			If Right(e.SortExpression, 1) = "+" Then
				...
				If SortBy.Length > 0 Then
					If SortBy.Contains("[" & e.SortExpression.Replace(" ", "_").Replace("+", "") & "]") Then
						' this sort order already set, so update.
						SortBy = SortBy.Replace("[" & e.SortExpression.Replace(" ", "_").Replace("+", "") & "]" &
						  If(e.SortDirection = SortDirection.Ascending, " Desc", " Asc"), "[" & e.SortExpression.Replace(" ", "_").Replace("+", "") & "]" &
						 If(e.SortDirection = SortDirection.Ascending, " Asc", " Desc"))
					Else
						SortBy &= "," & sortSql.Replace("+", "")
					End If
				Else
					SortBy = sortSql.Replace("+", "")
				End If
			...
			HttpContext.Current.Session(ID.Replace("Grid", "DATA")) = dataTable
			DataSource = dataTable
			DataBind()
		End If
	End Sub
	...
	Public Shared Function SortExpressionToSql(ByVal sortExpression As String, ByVal sortDir As Nullable(Of SortDirection)) As String
		Return _
		 ((If((sortExpression Is Nothing), Nothing, "[" & sortExpression & "]")) & " " &
		  sortDir.ToString.Replace("ending", ""))
	End Function
	...
	Public Shared Sub GridViewSort(ByVal sender As Object, ByVal e As GridViewSortEventArgs)
		...
		If previousColumn = e.SortExpression.Replace(" ", "_") Then
			...
			e.SortDirection =
			 Flip(
			  If _
					((previousDirection Is Nothing), SortDirection.Descending,
					 DirectCast([Enum].Parse(GetType(SortDirection), previousDirection), SortDirection)))
	...
	Function SetLookupFilter(ByVal dt As DataTable) As DataTable
		...
		If dt IsNot Nothing Then
			...
			'   "ISNULL([ASRSysLookupFilterValue], '') = 'HERTFORDSHIRE'"
	...
	Protected Sub PagerCommand(ByVal sender As Object, ByVal e As CommandEventArgs)
		...
		Select Case e.CommandName
			...
			Case "Previous"
				...
				End If
			Case "Next"
				If Not PageIndex = PageCount Then
					newPageIndex = PageIndex + 1
				End If
			Case "Last"
